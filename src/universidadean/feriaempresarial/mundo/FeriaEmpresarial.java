...
public class FeriaEmpresarial {
    ...
    int cantPuestosCentro =0;
    int cantPuestosNorte =0;
    int cantPuestosSur =0;
    int cantPuestosOccidente =0;
    int cantPuestosOriente =0;
    int cantPuestos = 0;
    String NombreZona = "";

        for (Puesto p: puestos){
        if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_CENTRO)){cantPuestosCentro++;}
        if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_NORTE)){cantPuestosNorte++;}
        if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_SUR)){cantPuestosSur++;}
        if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_OCCIDENTE)){cantPuestosOccidente++;}
        if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_ORIENTE)){cantPuestosOriente++;}
    }
        cantPuestos = cantPuestosCentro;
        NombreZona = "Zona Centro";
        if(cantPuestos < cantPuestosNorte){
            NombreZona = "Zona Norte";
            cantPuestos = cantPuestosNorte;
        }
        if(cantPuestos < cantPuestosSur){
            NombreZona = "Zona Sur";
            cantPuestos = cantPuestosSur;
        }
        if(cantPuestos < cantPuestosOccidente){
            NombreZona = "Zona Occidente";
            cantPuestos = cantPuestosOccidente;
        }
        if(cantPuestos < cantPuestosOriente){
            NombreZona = "Zona Oriente";
            cantPuestos = cantPuestosOriente;
        }
        return "Existen " + cantPuestos + " puestos ocupados en la " + NombreZona + ".";
